...
(define (etai key)
  (let*([top5 (take (stats:cipher-monograms utils:ciphertext) 5)]
        [AI (append* (map string->list (stats:cipher-common-words-single utils:cipher-word-list)))]
        ;[other3 (remove* AI top5)]
;        [top3-nbrhd (stats:unzip (take (stats:cipher-neighbourhood
;                                        (stats:cipher-bigrams-frq utils:cipher-word-list)
;                                        'successor) 3))]
        [the4 (map string->list (take (stats:cipher-common-words-triple utils:cipher-word-list) 4))]
        [E (map last the4)]
        [T (map first the4)])
    (define (add-a-i z) ;;;; z is a list of substitutions
      (list (cons (cons #\A (car AI)) z) (cons (cons #\I (car AI)) z)))
    ;(displayln (list the4 top5 AI E T))
    (if (= 1 (length AI))
        (lc (utils:add-substitution x key) : x <- (append* (map add-a-i (lc (list (cons #\E (cdr te)) (cons #\T (car te))) :
                                                                            te <- (stats:zip T E) ))))
        (lc (utils:add-substitution x key) : x <- (lc (list (cons #\E (cdr te)) (cons #\T (car te)) (cons #\A a) (cons #\I i)) :
                                                      te <- (stats:zip T E)  a <- AI   i <- AI 
                                                      @(not (char=? a i)))))))
