...
(define (crack-cipher strategies key) ;; Returns list of encryption keys.
  ...
  (cond [(null? strategies) key]
        [else (let* ([strategy (car strategies)]
                     [list-keys (strategy key)]
                     [modified-key (apply-DC-SWE  algo:dictionary-closure  list-keys 0)])
                (if modified-key
                    (cond [(compelete? modified-key) (list modified-key)]
                          [else (crack-cipher (cdr strategies) modified-key)])
                    (crack-cipher (cdr strategies) key)))]))

(define (apply-DC-SWE  dictionary-closure  list-keys  key-num)
  (cond [(null? list-keys) #f]
        [else (displayln (list "@@@ key " key-num))
              (let ([soln (dictionary-closure (car list-keys))])
                (if soln
                    (algo:secret-word-enumeration soln)
                    (apply-DC-SWE dictionary-closure (cdr list-keys) (add1 key-num))))]))
(define (compelete? key)
  (for/and ([ i key])
    (if(char=? i #\_) #f #t)))
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;;                                 Optional task                                    ;;
